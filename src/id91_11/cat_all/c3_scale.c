...
void ExecuteCompScale(const id0_byte_t *codePtr, id0_unsigned_t egaDestOff, const id0_byte_t *srcPtr, id0_byte_t mask)
{
	while (*codePtr != 0xcb)
	{
		...
		while (*codePtr == 0x26)
		{
			...
			BE_ST_EGAUpdateGFXBitsFrom4bitsPixel(egaDestOff+heightofs, srcVal, mask);
		}
	}
}
...
//
	...
	BE_ST_EGAUpdateGFXBufferInPlane(workEgaOff+offsetof(t_compshape, width), (id0_byte_t *)&width, sizeof(width), 0);
	//work->width = width;
...
//
	...
	for (x=firstline;x<=lastline;x++)
		for (y=0;y<64;y++)
			if (spotvis[y][x] != BACKGROUNDPIX)
				BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, spotvis[y][x], 0);
				//*code++ = spotvis[y][x];
...
//
	for (x=firstline;x<=lastline;x++)
	{
		...
		BE_ST_EGAUpdateGFXBufferInPlane(workEgaOff+offsetof(t_compshape, codeofs)+(x-firstline)*sizeof(id0_unsigned_t), (id0_byte_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 0);
		//BE_ST_EGAUpdateGFXBufferInPlane(workEgaOff+offsetof(t_compshape, codeofs[x-firstline]), (id0_byte_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 0);
		...
		// compile the scale call
		//
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x8b, 0);		// mov bx,[lastpix*2]
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x1e, 0);
			tempUnsignedWord = lastpix*2;
			BE_ST_EGAUpdateGFXBufferInPlane(codeEgaOff, (id0_byte_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 0);
			codeEgaOff += 2;

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x8b, 0);		// mov cx,[bx]
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x0f, 0);

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xc6, 0);		// move [BYTE bx],0xcb
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x07, 0);
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xcb, 0);

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xa1, 0);		// mov ax,[firstpix*2]	/*************
			tempUnsignedWord = firstpix*2;
			BE_ST_EGAUpdateGFXBufferInPlane(codeEgaOff, (id0_byte_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 0);
			codeEgaOff += 2;

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x36, 0);		// mov [ss:0],ax
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xa3, 0);
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x00, 0);
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x00, 0);

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x8e, 0);		// mov ds,dx	(mov ds,cs)
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xda, 0);

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xbe, 0);		// mov si,OFFSET pixelofs-firstpixel
			tempUnsignedWord = pixelofs-firstpix;
			BE_ST_EGAUpdateGFXBufferInPlane(codeEgaOff, (id0_byte_t *)&tempUnsignedWord, sizeof(tempUnsignedWord), 0);
			codeEgaOff += 2;

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0xff, 0);		// call [DWORD bp]
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x5e, 0);
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x00, 0);


			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x8e, 0);		// mov ds,[bp+2]
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x5e, 0);
			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x02, 0);

			BE_ST_EGAUpdateGFXByteInPlane(codeEgaOff++, 0x89, 0);		// mov [bx],cx
...
#endif
	MM_GetPtr ((memptr *)finalspot,totalsize);
	BE_ST_EGAFetchGFXBufferFromPlane((id0_byte_t *)*finalspot, workEgaOff, totalsize, 0);
	//_fmemcpy ((id0_byte_t id0_seg *)(*finalspot),(id0_byte_t id0_seg *)work,totalsize);
//	MM_FreePtr ((memptr *)&work);
...
//
// Includes safe unaligned accesses; Note that since firstpixtimestwo and
// lastpixtimestwo shall be even numbers, and comptablebuff should point
// to a properly-aligned memory chunk (originally the start of a segment),
// a few 8-bit pointers may be dereferenced as 16-bit.
static void ExecuteCompShape(const id0_byte_t *codePtr, id0_byte_t *comptablebuff, id0_unsigned_t egaDestOff, const id0_byte_t *srcGfxPtr, id0_byte_t mask)
{
	...
	while (*currCodePtr != 0xcb)
	{
		...
		//
		// mov bx,[lastpix*2]
		id0_int_t lastpixtimestwo;
		memcpy(&lastpixtimestwo, currCodePtr+2, 2);
		id0_byte_t *scalerFarRetPtr = comptablebuff + *(id0_int_t *)(comptablebuff+lastpixtimestwo);
		...
		*scalerFarRetPtr = 0xcb; // far return
		...
		// call [DWORD bp]
		id0_int_t firstpixtimestwo;
		id0_int_t pixelofsdiff;
		memcpy(&firstpixtimestwo, currCodePtr+10, 2);
		memcpy(&pixelofsdiff, currCodePtr+19, 2);
		ExecuteCompScale(comptablebuff + *(id0_int_t *)(comptablebuff+firstpixtimestwo), egaDestOff, srcGfxPtr+pixelofsdiff, mask);
		// mov ds,[bp+2]
		//
		// unpatch far return:
		// mov [bx],cx
